...
namespace GridMC
{
   ...
   */
   void McSystem::readParam(istream& in)
   {
      ...
      sscanf(line.c_str(), "%lf %lf %s", &esStrength_, &esRatio_, comment);
   ...
   */
   void McSystem::calculateStatusVariable()
   {
      ...
      double Qi, permittivity, aBorn;

      // Ion radii assumed to be half the lattice spacing (for the moment).
      aBorn = boxL_[0] / double(nGrid_[0]) / 2.0;
      ...
      solvationEnergy_ *= esStrength_ / (2.0 * aBorn);
   ...
   */
   double McSystem::calculateMoveWeight(const int id, const Vector& rTrial)
   {
      ...
      if (fabs(Qi) > Constants::Epsilon) {
         double aBorn, permittivity;

         aBorn = boxL_[0] / double(nGrid_[0]) / 2.0;

         permittivity = grid_.getDielectricPermittivity(beads_[id].r);
         dSolvationEnergy_ = -esStrength_ * Qi * Qi / (2.0 * permittivity * aBorn);
